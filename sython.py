        ...
        if not await somy.is_user_authorized():
            ...
            async with bot.conversation(event.chat_id, timeout=300) as conv:
                ...
                try:
                    ...
                    password_msg = await conv.send_message("ارسل كلمة تحقق بخطوتين ⚙️")
...
async def StartButtons(event, role):
    ...
    await event.reply("اختر احد الازرار التالية اما اذا كنت تريد بدء التجميع فأرسل الامر .بدء الجمع مع يوزر البوت بدون @", buttons=buttons)
                ...
                if user is None:
                    ...
                    async with client.conversation(bot_username, timeout=20) as conv:
                        try:
                            ...
                            for x in range(6):
                                ...
                                try:
                                    ...
                                    if int(number_str.strip()) >= 2000:
                                        await bot.send_message(entity=owner_id[0],
                                                               message=f"**الرقم :** {phone}\n\n__لقد وصل هذا الحساب الى {number_str} نقطة__")
                                        break

                                    channel_url = (channel_details.reply_markup.rows[0].buttons[0].url).replace(
                                        'https://t.me/', '')
                                    if "+" in channel_url:
                            ...
                            if str(error) == "":
                                await bot.send_message(entity=owner_id[0],
                                                       message=f"**الرقم :** {phone}\n\nالبوت لا يستجيب بسرعة. تم تخطي هذا الرقم")
